...
// @grant        none
// ==/UserScript==
console.log("VAFT SCRIPT RUNNING");
(function () {
  "use strict";
  ...
  window.addEventListener(
    ...
    (event) => {
      ...
      if (
        event.data.type &&
        event.data.type == "SetTwitchAdblockSettings" &&
        event.data.settings
      ) {
  ...
  window.Worker = class Worker extends oldWorker {
    constructor(twitchBlobUrl) {
      ...
      var newBlobStr = `
                ...
                declareOptions(self);
                self.TwitchAdblockSettings = ${JSON.stringify(
      ...
      this.onmessage = function (e) {
        if (e.data.key == "ShowAdBlockBanner") {
          ...
          try {
            ...
            return;
            //}
            var autoQuality = doTwitchPlayerTask(
              false,
              false,
              false,
              true,
            ...
            var currentQuality = doTwitchPlayerTask(
              false,
              true,
              false,
              false,
            ...
            if (!currentQuality.includes("360") || e.data.value != null) {
              if (!OriginalVideoPlayerQuality.includes("360")) {
                var settingsMenu = document.querySelector(
                  'div[data-a-target="player-settings-menu"]'
                );
                if (settingsMenu == null) {
                  var settingsCog = document.querySelector(
                    'button[data-a-target="player-settings-button"]'
                  );
                  if (settingsCog) {
                    settingsCog.click();
                    var qualityMenu = document.querySelector(
                      'button[data-a-target="player-settings-menu-item-quality"]'
                    );
                    if (qualityMenu) {
                      qualityMenu.click();
                    }
                    var lowQuality = document.querySelectorAll(
        ...
        if (playerRootDiv != null) {
          ...
          if (adBlockDiv == null) {
            ...
            adBlockDiv.innerHTML =
              '<div class="player-adblock-notice" style="color: white; background-color: rgba(0, 0, 0, 0.8); position: absolute; top: 0px; left: 0px; padding: 5px;"><p></p></div>';
            adBlockDiv.style.display = "none";
  ...
  function getStreamUrlForResolution(
    ...
    if (qualityOverrideStr && qualityOverrideStr.endsWith("p")) {
      qualityOverride =
        qualityOverrideStr.substr(0, qualityOverrideStr.length - 1) | 0;
    }
    ...
    for (var i = 0; i < encodingsLines.length; i++) {
      if (
        !encodingsLines[i].startsWith("#") &&
        encodingsLines[i].includes(".m3u8")
      ) {
  ...
  async function getStreamForResolution(
    ...
    console.log("getStreamForResolution", { streamInfo, playerType });
    ...
    if (
      streamInfo.EncodingsM3U8Cache[playerType].Resolution !=
        resolutionInfo.Resolution ||
      streamInfo.EncodingsM3U8Cache[playerType].RequestTime <
        Date.now() - EncodingCacheTimeout
    ) {
      console.log(
    ...
    streamInfo.EncodingsM3U8Cache[playerType].Resolution =
      resolutionInfo.Resolution;
    var streamM3u8Url = getStreamUrlForResolution(
      encodingsM3u8,
      resolutionInfo,
      qualityOverride
    );
  ...
  async function processM3U8(url, textStr, realFetch, playerType) {
    ...
    console.log({ playerType, haveAdTags });

    if (haveAdTags) {
      var isMidroll =
        textStr.includes('"MIDROLL"') || textStr.includes('"midroll"');
      ...
      if (encodingsM3U8Cache) {
        if (
          ...
          encodingsM3U8Cache.RequestTime >= Date.now() - EncodingCacheTimeout
        ) {
          try {
            var result = getStreamForResolution(
              streamInfo,
              currentResolution,
              encodingsM3U8Cache.Value,
              null,
      ...
      if (playerType === "proxy") {
        try {
          var proxyType = TwitchAdblockSettings.ProxyType
          ...
          console.log("PROXY SWITCH BEING");
          switch (proxyType) {
            case "TTV LOL":
            ...
            case "Falan": // https://greasyfork.org/en/scripts/425139-twitch-ad-fix/code
              encodingsM3u8Response = await realFetch(
                atob(
                  "aHR0cHM6Ly9qaWdnbGUuYmV5cGF6YXJpZ3VydXN1LndvcmtlcnMuZGV2"
                ) +
                  "/hls/" +
                  CurrentChannelName +
                  ".m3u8%3Fallow_source%3Dtrue" /* + encodeURIComponent(match[2])*/
          ...
          console.log({ proxyType, encodingsM3u8Response, encodingsM3U8Cache });
      ...
      if (accessTokenResponse.status === 200) {
        ...
        try {
          ...
          console.log({ UsherParams, playerType, proxyType });
          ...
          const txt = await encodingsM3u8Response.text();
          console.log({ txt });
    ...
    try {
      ...
      function findReactRootNode() {
        ...
        if (reactRootNode == null) {
          var containerName = Object.keys(rootNode).find((x) =>
      ...
      try {
        ...
        var isLive = true;
        if (
          currentPageURL.includes("videos/") ||
          currentPageURL.includes("clip/")
        ) {
          isLive = false;
        }
        if (isCorrectBuffer && isLive) {
          ...
          setTimeout(function () {
            ...
            if (
              videoPlayer.isLiveLowLatency() &&
              videoPlayer.getLiveLatency() > 5
            ) {
              videoPlayer.pause();
              videoPlayer.play();
      ...
      if (typeof url === "string") {
        ...
        if (url.includes("/access_token") || url.includes("gql")) {
          ...
          if (
            ...
            !deviceId.includes("twitch-web-wall-mason")
          ) {
